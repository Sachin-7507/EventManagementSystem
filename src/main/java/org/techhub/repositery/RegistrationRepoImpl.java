...
public class RegistrationRepoImpl extends DBInitialize implements RegistrationRepo {
	...
	public List<Map<String, String>> getUserRegistrationPDFData(int studentId) {
		...
		try {
			stmt = conn.prepareStatement(
					"SELECT e.name AS eventName, e.venue, e.eventdate, r.regdate " + "FROM registration r "
							+ "INNER JOIN campuseventdb e ON r.eventid = e.eventid " + "WHERE r.studentid = ?");
			stmt.setInt(1, studentId);
			rs = stmt.executeQuery();
	...
	public List<EventModel> getEventCapacity() {
		System.out.println("ID"+"\t"+"name"+"\t\t   "+"Capacity"+"\t"+"Total Registration");
		List<EventModel> ls = new ArrayList<>();
		try {
			stmt = conn.prepareStatement(
					"select e.eventid , e.name , e.capacity ,(select count(*) from registration r where e.eventid = r.eventid) as TotalReg from campuseventdb  e ");
			rs = stmt.executeQuery();
		//	while (rs.next()) {
				/*
				 * EventModel em = new EventModel(); em.setEventid(rs.getInt(1));
				 * em.setName(rs.getString(2)); em.setCapacity(rs.getInt(3));
				 */
				//int totalReg = rs.getInt("TotalReg");
				
				while (rs.next()) {
					System.out.println(rs.getInt("eventid") + "\t" + rs.getString("name") + "\t\t"
							+ rs.getInt("capacity") + "\t\t" + rs.getInt("TotalReg"));
				}
				//ls.add(em);
			}
